...
####Helper Functions####
##Pretty Dates
format_pretty_date <- function(date, use_suffix = TRUE, language = "en") {
  date <- as.Date(date)
  day_num <- lubridate::day(date)
  
  day_str <- if (use_suffix && language == "en") {
  ...
  if (language == "es") {
    month_name <- spanish_months[as.numeric(format(date, "%m"))]
    formatted <- paste0(day_str, " de ", month_name, " de ", format(date, "%Y"))
  } else {
    formatted <- paste0(
      format(date, "%B "),
      day_str,
  ...
  list(
    question =
      tags$div(
        ...
        tags$p(
          "Were any of the charges for which you were sentenced to probation a felony? If you don't know the answer to this, you can look that up ",
          tags$a(href='https://ujsportal.pacourts.us/casesearch', target='_blank', class='external-link', "here"),
          "."
        )
      ),
...
ui <- fluidPage(
  ...
  div(strong(uiOutput("main_app_title")),
...
server <- function(input, output, session){
  ...
  get_english_key <- function(translated_answer_from_input, current_lang_code){
    if(is.null(translated_answer_from_input) || nchar(translated_answer_from_input) == 0){
      return(NULL)
    }

    if(current_lang_code == "en"){
      return(translated_answer_from_input)
    }else if(current_lang_code == "es"){
      translation <- 
        as.data.frame(i18n_r()$get_translations()) %>% 
        rownames_to_column("en") %>% 
        filter(es == translated_answer_from_input) %>% 
        pull(en)
      return(translation)
    }
  }
  ...
  output$app_title <- renderText({
    i18n_r()$t("Act 44 Early Termination Tool")
  })
  
  output$main_app_title <- renderText({
    i18n_r()$t("Act 44 Early Termination Tool")
  })
  ...
  calculate_eligibility_date_prc <- reactive({
    ...
    if(!is.null(probation_start_date())){
      base_date <- as.Date(probation_start_date())
      # base_date = probation_start_date()
    }else{
      base_date <- as.Date(sentencing_date())
      # base_date = sentencing_date()
    }
    felony_val <- felony_or_misdemeanor()
    ...
    probation_after_parole_answer_val <- probation_after_parole_answer() 
    ...
    if(felony_val == "Misdemeanor" & (multiple_misdemeanors_val == "No" | (same_conduct_val == "Same Conduct" & multiple_misdemeanors_val == "Yes"))){
      ...
      if(probation_after_parole_answer_val == "Yes" & final_year_of_parole_val == "Yes"){
  ...
  render_section_7_q3 <- reactive({
    ...
    question_content <- tags$div(
      tags$p(HTML(paste0(
        ...
        "<strong>", format_pretty_date(eligibility_date_val - months(6), language = input$selected_language), "</strong>",
        i18n_r()$t(" to "), "<strong>", format_pretty_date(eligibility_date_val, language = input$selected_language), "</strong>",
  ...
  render_section_7_act_44_relief_result <- reactive({
    eligibility_date_val <- eligibility_date_section_7()
    eligibility_date_val_clean <- format_pretty_date(eligibility_date_val, language = input$selected_language)
  ...
  render_prc_flow_q7 <- reactive({
    ...
    question_content <-
      tags$div(
        tags$p(HTML(paste0(i18n_r()$t("On or before "), "<strong>", format_pretty_date(as.Date(eligibility_date_val) - days(30), language = input$selected_language), "</strong>",
        ...
        tags$p(HTML(paste0(i18n_r()$t("If you did not receive this report by "),
                           "<strong>", format_pretty_date(as.Date(eligibility_date_val) - days(30), language = input$selected_language), "</strong>",
    ...
    question_content <-
      tags$div(
        tags$p(HTML(paste0(i18n_r()$t("Your Probation Review Conference must be held by "),
                           "<strong>", format_pretty_date(as.Date(eligibility_date_val) + days(60), language = input$selected_language), "</strong>",
  ...
  render_prc_flow_q10 <- reactive({
    ...
    tagList(
      translate_ui_recursive(question_content, i18n_r),
      fluidRow(
        column(width = 12,
               div(
  ...
  ##Translate UI Recursive Function
  translate_ui_recursive <- function(element, translator_reactive){
    if(is.character(element)){
      ...
      if(!is.null(element$attribs)){
        ...
        if(element$name %in% c("strong", "b") && !is.null(element$children) && is.character(element$children[[1]])){
          element$children[[1]] <- translator_reactive()$t(element$children[[1]])
        }
      }
      return(element)
    ...
    if(length(current_question$intro) > 0){
      ...
      tagList(
        ...
        if("choices" %in% names(current_question) &&
            length(current_question$choices) > 0 &&
        ...
        }else if("choices" %in% names(current_question) &&
                   length(current_question$choices) > 0 &&
                   length(current_question$date_question) > 0){
          dateInput(
            inputId = "answer",
      ...
      tagList(
        ...
        current_question %>%
          ...
          pmap(
            ~tagList(
              div(tags$div(tags$p(translate_ui_recursive(.y, i18n_r)))), 
              radioButtons(
  ...
  observeEvent(input$answer, {
    ...
    if(length(current_question$question_list) == 0 && length(current_question$date_question) > 0){
      ...
      if(!is.null(input$answer_years) && nchar(input$answer_years) > 0 &&
          !is.null(input$answer_months) && nchar(input$answer_months) > 0){
      ...
      english_key_to_store <- get_english_key(input$answer, current_lang_code)
      
      if(current_question$question_id %in% c("section_7_q2", "prc_flow_q2")){
        ifelse(english_key_to_store == english_key_to_store, felony_or_misdemeanor("Felony"), felony_or_misdemeanor("Misdemeanor"))
      }else if(current_question$question_id == "prc_flow_q3_2"){
        ifelse(english_key_to_store == "Yes", multiple_misdemeanors("Yes"), multiple_misdemeanors("No"))
      }else if(current_question$question_id == "prc_flow_q5"){
        ifelse(english_key_to_store == "Yes", education_credits_answer("Yes"), education_credits_answer("No"))
      }else if(current_question$question_id == "prc_flow_q6"){
        ifelse(english_key_to_store == "Yes", education_credits_more_than_2_answer("Yes"), education_credits_more_than_2_answer("No"))
      }else if(current_question$question_id == "prc_flow_q3_1"){
        ifelse(english_key_to_store == "Yes", probation_after_parole_answer("Yes"), probation_after_parole_answer("No"))
      }else if(current_question$question_id == "prc_flow_q3_1_final_year_parole"){
        ifelse(english_key_to_store == "Yes", final_year_of_parole_answer("Yes"), final_year_of_parole_answer("No"))
      }else if(current_question$question_id %in% c("prc_flow_split_sentence", "section_7_split_sentence")){
        if(english_key_to_store == "No"){ 
          probation_start_date(NULL)
        }
      }
      ifelse(is.null(english_key_to_store) == FALSE, answer_selected(TRUE), answer_selected(FALSE))
      selected_answer(english_key_to_store)
      ...
      responses <- sapply(answer_ids, function(id) get_english_key(input[[id]], current_lang_code))  
      null_responses <- 
        responses %>% 
        map(~is.null(.x)) %>% 
        unlist() %>% 
  ...
  observeEvent(input$next_button, {
    ...
    current_question <- decision_tree[[current_index]]
    answer_selected_val <- answer_selected()
    if(answer_selected_val == FALSE && !("result" %in% names(current_question)) && !(length(current_question$next_question) == 1 && !("choices" %in% names(current_question)))){
      shinyalert(
    ...
    if(current_question$question_id == "defendant_sentencing_date"){
      req(sentencing_date()) 
      if(as.Date(sentencing_date()) >= as.Date("2024-06-11")){
      ...
      if(length(current_question$question_list) == 0){
        if(current_question$question_id == "section_7_q3"){
          ...
          req(technical_violation_found_answer()) 
          ...
          req(education_credits_answer(), felony_or_misdemeanor()) 
          education_credits_val <- education_credits_answer()
