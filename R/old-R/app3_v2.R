...
server <- function(input, output, session) {
  ...
  output$quiz_ui <- renderUI({
    ...
    if (!is.null(current_question$question) && !is.null(current_question$choices) && length(current_question$choices) > 0) {
      radioButtons(
        "answer",
        current_question$question,
        choices = current_question$choices
      )
    } else if (!is.null(current_question$result)) {
  ...
  output$button_ui <- renderUI({
    current_question <- decision_tree[[current_question_id()]]
    if (!is.null(current_question$result)) {
  ...
  observeEvent(input$next_button, {
    if (!is.null(selected_answer())) {
      ...
      # Find the index of the next question or result
      next_question_index <- which(sapply(decision_tree, function(x) {
        !is.null(x$question_id) && x$question_id == next_id
      }))
      
      if (length(next_question_index) > 0) {
        current_question_id(next_question_index)
      } else {
        # If it's a result, find and display it
        result_index <- which(sapply(decision_tree, function(x) {
          !is.null(x$question_id) && x$question_id == next_id
        }))
        
        if (length(result_index) > 0) {
          current_question_id(result_index)
        }
      }
