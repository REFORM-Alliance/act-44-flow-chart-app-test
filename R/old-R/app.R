...
server <- function(input, output, session){
  ...
  # observe({
  #   req(input$selected_language)  # ensure input is available
  #   i18n$set_translation_language(input$selected_language)
  #   
  #   # Send updated title to browser tab
  #   session$sendCustomMessage("update-title", i18n$t("Act 44 Early Termination Tool"))
  # })
  ...
  output$quiz_ui <- renderUI({
    ...
    if (length(current_question$intro) > 0) {
      ...
      # Section: Special question_id overrides
      ...
      # Section: Single question (radio/date/dropdown)
      ...
      # Section: Multi-question pages (question_list)
    } else if ("question" %in% names(current_question) &&
               length(current_question$question_list) > 0 &&
               length(current_question$date_question) == 0) {
      tagList(
