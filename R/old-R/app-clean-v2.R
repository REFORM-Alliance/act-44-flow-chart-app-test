...
decision_tree <- list(
  ...
  list(
    # question =
    #  tags$div(
    #   tags$p(paste0("Now we need to ask you about the probationer's behavior in the following dates: ", format_pretty_date(as.Date(eligibility_date() - month(6))), " to ", format_pretty_date(eligibility_date()),
    ...
    next_question = list(
      "JUDGE FINDS TECHNICAL VIOLATIONS" = "no_act_44_relief_yet_result",
      "NO TECHNICAL VIOLATIONS" = "no_act_44_relief_yet_result"
    )
  ),
  list(
...
server <- function(input, output, session) {
  ...
  calculate_eligibility_date <- reactive({
    if (!is.null(values$sentencing_date) & !is.null(values$felony_or_misdemeanor)) {
      base_date <- as.Date(values$sentencing_date)
      felony_val <- values$felony_or_misdemeanor
  ...
  render_before_june_11_q3 <- reactive({
    ...
    if (!is.null(eligibility_date_val)) {
      question_content <- tags$div(
        tags$p(paste0("Now we need to ask you about the probationer's behavior in the following dates: ", format_pretty_date(eligibility_date_val - months(6)), " to ", format_pretty_date(eligibility_date_val),
                      ". A judge will look at the probationer's record and determine whether they have committed any technical violations during this time period. If they do, it changes what relief the probationer can receive. A technical violation is defined as the violation of any specific term of probation that is not a criminal conviction.")),
        tags$p("A judge will assess whether the probationer has committed a violation in the following categories:"),
        tags$ol(type = "I",
                tags$li("A violation that was sexual in nature"),
  ...
  violation_answer <- reactiveVal(NULL) # Store the selected answer
  ...
  observeEvent(input$answer, {
    ...
    if (length(current_question$question_list) == 0 & length(current_question$date_question) > 0) {
      if (current_question$question_id == "defendant_sentencing_date") {
        values$sentencing_date <- as.Date(input$answer)
      ...
      if (current_question$question_id == "before_june_11_q2") {
        ifelse(input$answer == "Yes", values$felony_or_misdemeanor <- "Felony", values$felony_or_misdemeanor <- "Misdemeanor")
      }
